                            ...
                            <asp:ListView runat="server" ID="MediaFieldsList" OnItemDataBound="MediaFieldsList_ItemDataBound" ItemType="FrameworkLibrary.MediaDetailField">
                                ...
                                <ItemTemplate>
                                    <asp:Panel runat="server" CssClass="field" ID="FieldWrapper">
                                        ...
                                        <%# (!Item.RenderLabelAfterControl) ? "<label data-fieldcode='" + Item.FieldCode + "' data-fieldid=" + Item.ID + ">" + Item.FieldLabel + "</label> <i class='fa fa-question-circle tooltip' aria-hidden='true' title='" + Item.FieldDescription + "'></i><div class='floatRight'><small>USAGE: {Field:" + Item.FieldCode + "} OR {{Load:" + ((Item.MediaDetail != null)? Item.MediaDetail.MediaID.ToString() : "[MediaID]") +"}.Field:"+Item.FieldCode+"}</small></div>" : "" %>
