...
namespace WebApplication.Admin.Roles
{
    public partial class Default : AdvanceOptionsBasePage
    {
        private List<Role> Items
        {
            get
            {
                return (List<Role>)ContextHelper.GetFromRequestContext(ItemList.ClientID);
            }
            set
            {
        ...
        {
            this.Section.Text = this.Page.Title = "Manage Roles";

            if (Items == null)
                Items = BaseMapper.GetDataModel().Roles.OrderBy(i => i.Name).ToList();
